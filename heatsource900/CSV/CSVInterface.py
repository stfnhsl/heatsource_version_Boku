...
class CSVInterface(object):
    ...
    def ReadControlFile(self, inputdir, control_file):
        ...
        into the Initialization dictionary {IniParams} and does some formatting"""
    
        if not exists(join(inputdir, control_file)):
            raise Exception("HeatSource_Control.csv not found. "
                            "Move the executable or place the control file in this directory: %s." % inputdir)
        ...
        # BOKU
        IniParams["hydvel"] = IniParams["hydvel"] in "TRUE"
    ...
    def OrientNodes(self):
        ...
        for i in xrange(len(l)):
            ...
            # check for a zero slope.
            # We store all of them before checking so we can print a lengthy error that no-one will ever read.
            if self.Reach[key].S <= 0.0:
                slope_problems.append(key)
        # zeros are alright in shade calculations
        if self.run_type != 1:
            if len(slope_problems):
                raise Exception("The following reaches have zero slope. "
                                "Kilometers: %s" % ",".join(['%0.3f'%i for i in slope_problems]))
    ...
    def GetColumnarData(self):
        ...
        # Ignore in the loop, set them manually
        ignore = ["FLIR_Temp", "FLIR_Time"]
        # ###BOKU
        hydro = ["hydro_vel", "hydro_used"]

        data = {}
        
        # Get all the "columnar data" from the csv files.
        # Note Stream Km is brought in as an index file and set as data type object.
        # This is not the a perfects method
        # but it is done to avoid trailing on floating point values messing with the dictionary.
        # TODO organize the reading of this data into seperate methods so it is easier to find later.
        lcdata = pd.read_csv(join(IniParams["inputdir"], IniParams["lcdatafile"]),
                             quotechar='"', quoting=0)
        morphdata = pd.read_csv(join(IniParams["inputdir"], IniParams["morphfile"]),
                                quotechar='"', quoting=0, index_col='Stream_KM')
        accdata = pd.read_csv(join(IniParams["inputdir"], IniParams["accretionfile"]),
                              quotechar='"', quoting=0, index_col='Stream_KM')
        # ###BOKU
        ...
        # ###BOKU
        for i in xrange(len(hydro)):
            data[hydro[i]] = list(hydroveldata.ix[:, i])
        ...
        if max(data[ttools[1]]) > 180:
            ...
            raise Exception("Longitude must be less than 180 degrees."
                            " At rKM = " + str(rkm) + ", longitude = " + str(max1))
        if min(data[ttools[1]]) < -180:
            long_list = list(data[ttools[1]])
            min1 = min(long_list)
            index1 = long_list.index(min1)
            rkm = data[ttools[1]][index1]
            raise Exception("Longitude must be greater than -180 degrees."
                            " At rKM = " + str(rkm) + ", longitude = " + str(min1))
        # Latitude check

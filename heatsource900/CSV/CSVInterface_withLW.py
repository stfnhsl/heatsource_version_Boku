...
class CSVInterface(object):
    ...
    def SetupInputFiles(self, inputdir, control_file):
        ...
        climatefile = pd.DataFrame(index=timelist, columns=['GlobalRadiation', 'WindSpeed','RelativeHumidity','AirTemp', 'LwRad']*int((IniParams["contsites"]/len(climatefiles))))
        if IniParams["inflowsites"] > 0:
    ...
    def GetContinuousData(self):
        ...
        data = [tuple(zip(line[0:None:5],line[1:None:5],line[2:None:5],line[3:None:5],line[3:None:5])) for line in climatedata.values]
        ...
        for time in timelist:
            ...
            for air, glorad, lwrad, humid, wind in line:  #TODO: Achtung Reihenfolge geaendert
                i = c.next()
                #print (i)
